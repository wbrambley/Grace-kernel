...
u32 sci_record_data_save(SCI_LOG_MODE log_mode)
{
    ...
    while(((*pExcData)<(u32)SCI_EVENT_MAX) && (0 !=(*pExcData)))
    {
        ...
        /* coverity[dereferencing] */      
        sci_record_format(&tmp_buf,"current time stampe:  0x%x\n", *pExcData);      
        sci_record_flush2file(fd,tmp_buf_start,&tmp_buf);
        pExcData +=1;
    ...
    while(((*pucData)==(u8)SCI_DATA_SEND_EVENT) || ((*pucData)==(u8)SCI_DATA_RCV_EVENT))
    {
        ...
        {
            sci_record_format(&tmp_buf,"SEND:  timestamp:   ");
        }
        else/*receive data*/
        {
            sci_record_format(&tmp_buf,"RCVE:  timestamp:   ");
        }
        sci_record_format(&tmp_buf,"0x%.4x     ",(u32*)pucData);
        pucData += 4;
	...
	while(0!=(*(pExcData+1)))
	{
        ...
        sci_record_format(&tmp_buf,"tick is  0x%.8x, timestampe is 0x%.8x\n", *(pExcData), *(pExcData+1));
        sci_record_flush2file(fd,tmp_buf_start,&tmp_buf);
		pExcData +=2;
